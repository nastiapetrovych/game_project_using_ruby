...
import argparse
"""
The creation of web-map
"""
parser = argparse.ArgumentParser(description='This is my argparses')
...
def reading_file(path, year):
    ...
    Here I read file and return list of lists
    :param path: str
    :param year: str
    :return: list
    """
...
def coordinates(lst_with_locations_, my_coordinates):
    """
    Here I do everything: find the distance, create dictionary with keys of years, select a given year, create dict
    where key is name of film and values - list of coordinates in tuple and distance(sorted by distance)
...
def creating_map(locations, my_latitude, my_longitude):
    ...
    for element in locations:
        ...
        fg.add_child(
            folium.Marker(location=[latitude, longitude], popup=element[1][-1][0], icon=folium.Icon(icon='film', color="pink")))
    map.add_child(fg)
